...
// Five cards, nothing else. Recognition over recall: the cards ARE the prompt.
...
import { startSabotage, habitFollowupScreen } from "./flow-sabotage.js";
...
const cards = [
  ...
  { title: "There's something I keep not doing", sub: "Why, then a plan", tone: 2, go: () => startSabotage() },
  { title: "Just checking in", sub: "Thirty seconds", tone: 1, go: startCheckin },
];

function home(screen) {
  const planFollowups = db.duePlanFollowups;
  const habitFollowups = db.dueHabitFollowups;
  ...
  screen.append(
    ...
    // Quiet follow-up rows (next-day check-backs). Tonal, never loud, one each.
    ...planFollowups.slice(0, 1).map(p => listRow({
      glyph: "?",
      title: "That plan you logged — how did it actually go?",
      meta: "One slider, ten seconds",
      onClick: () => nav.go(followupScreen(p)),
    })),
    ...habitFollowups.slice(0, 1).map(h => listRow({
      glyph: "?",
      title: `${h.target_text || "Your plan"} — did it happen?`,
      meta: "One tap, no score",
      onClick: () => nav.go(habitFollowupScreen(h)),
    })),
